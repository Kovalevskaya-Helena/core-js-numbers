...
function getLinearEquationRoot(a, b) {
  if (a === 0) return 0;
  return (Math.sign(b) === -1 ? Math.abs(b) : -b) / a;
}
 ...
 *   1  => 1
 */
function getSumToN(length) {
  return Array.from({ length }, (_, index) => index + 1).reduce(
    (acc, n) => acc + n,
    0
  );
}
...
function getSumOfDigits(num) {
  ...
  return arr.reduce((acc, n) => Number(acc) + Number(n), 0);
}
